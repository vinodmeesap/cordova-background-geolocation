...
| [`notificationIcon`](#param-string-notificationicon-appicon) | `String` | Optional **Android** | `app icon` | When running the service with `foregroundService: true`, Android requires a persistent notification in the Notification Bar.  This allows you customize that icon.  Defaults to your application icon.|
